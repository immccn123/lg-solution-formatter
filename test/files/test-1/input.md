...
可能不太众所周知地，`libstdc++`在`<ext/numeric>`提供了一个`__gnu_cxx::power`函数。看 [实现](https://gcc.gnu.org/onlinedocs/gcc-9.5.0/libstdc++/api/a18031_source.html) 应该是一个快速幂的形状。然而截至GCC 14.2这东西现在还没有文档，但是这并不意味着我们就不会用这个东西了。
