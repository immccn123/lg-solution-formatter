/**
 * lg-solution-formatter v0.1.0 - a luogu solution formatter
 * Copyright (c) 2023-2023, Imken Luo <me@imken.moe> (MIT Licensed)
...
const CJKRegexStr = /[\x31F0-\x31FF\x3220-\x3243\x3280-\x337F]|[\u30A0-\u30FF]|[\u3041-\u3096]|[\u4E00-\u9FCC\u3400-\u4DB5\uFA0E\uFA0F\uFA11\uFA13\uFA14\uFA1F\uFA21\uFA23\uFA24\uFA27-\uFA29]|[\ud840-\ud868][\udc00-\udfff]|\ud869[\udc00-\uded6\udf00-\udfff]|[\ud86a-\ud86c][\udc00-\udfff]|\ud86d[\udc00-\udf34\udf40-\udfff]|\ud86e[\udc00-\udc1d]/;
const CJKPunctuation = /[，。【】「」『』❲❳［］（）《》！？“”、～；：]/;
const nonCJKBracket = /[\(\)\[\]\{\}]/;
const nonCJKLBracket = /[\(\[\{]/;
const toFullWidth = /[!\?\.,:;]/;
const CJKRgx = (regexStr, flags) => {
    return RegExp(regexStr.replaceAll("{CJK}", CJKRegexStr.source), flags);
};
const isCJK = (char) => CJKRegexStr.test(char);
const isSpace = (char) => char === " ";
const isPunctuation = (char) => CJKPunctuation.test(char);
const isNonCJKLBracket = (char) => nonCJKLBracket.test(char);
const isNonCJKRBracket = (char) => nonCJKBracket.test(char) && !nonCJKLBracket.test(char);
const shouldAddSpace = (last, now) => {
    // 中文标点
    if (isPunctuation(last) || isPunctuation(now))
        return false;
    // 已经存在空格
    if (isSpace(last) || isSpace(now))
        return false;
    // 汉字和汉字之间
    if (isCJK(last) === isCJK(now))
        return false;
    // 半角括号和中文
    if (isNonCJKLBracket(last) || isNonCJKRBracket(now))
        return false;
    return true;
};
const shouldAddSpaceBetweenTokens = (last, now, lastRemoveSpace) => {
    if (isCJK(last) && isCJK(now))
        return lastRemoveSpace;
    if (!isCJK(last) && !isCJK(now))
        return lastRemoveSpace;
    return shouldAddSpace(last, now);
};
const shouldRemoveSpace = (last, next) => {
    // 多数情况下，中文标点前后不需要空格。
    if (last && isPunctuation(last))
        return true;
    if (next && isPunctuation(next))
        return true;
    // 半角左括号的右边不需要
    if (last && isNonCJKLBracket(last))
        return true;
    // 半角右括号的左边不需要
    if (next && isNonCJKRBracket(next))
        return true;
    // 多个空格留一个
...
const shouldFullWidth = (last, now) => {
    if (isCJK(last))
        return toFullWidth.test(now);
    return false;
};
const getFullWidth = (now) => {
    return (() => {
        switch (now) {
            case ".":
                return "。";
...
const formatString = (text) => {
    ...
    for (let i = 0; i < text.length; i++) {
        ...
        if (text[i] === " ") {
            if (shouldRemoveSpace(out[out.length - 1], text[i + 1]))
                continue;
        }
        if (!isReplaced)
            out += text[i];
...
class _Lexer {
    ...
    inlineTokens(src, tokens = []) {
        ...
        if (this.tokens.links) {
            ...
            if (links.length > 0) {
                while ((match = this.tokenizer.rules.inline.reflinkSearch.exec(maskedSrc)) !=
                    null) {
        ...
        while ((match = this.tokenizer.rules.inline.anyPunctuation.exec(maskedSrc)) !=
            null) {
            maskedSrc =
                maskedSrc.slice(0, match.index) +
                    "++" +
        ...
        while (src) {
            ...
            const katexCutter = (src) => {
                const katexInlineStartRule = /(?<=\s|^)\${1,2}(?!\$)/;
                const katexInlineRule = /^(\${1,2})(?!\$)((?:\\.|[^\\\n])+?)(?<!\$)\1(?=[\s?!\.,:]|$)/;
...
class _Renderer {
    ...
    link(href, title, text) {
        ...
        href = cleanHref;
        const out = `[${formatString(text)}](${href}${title ? '"' + formatString(title) + '"' : ""})`;
        return out;
    }
...
class _Parser {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            switch (token.type) {
                ...
                case 'heading': {
                    ...
                    mdText += this.renderer.heading(this.parseInline(headingToken.tokens).mdText, headingToken.depth, unescape(this.parseInline(headingToken.tokens, this.textRenderer).mdText));
                    continue;
                }
                case 'code': {
                    const codeToken = token;
                    mdText += this.renderer.code(codeToken.text, codeToken.lang, !!codeToken.escaped);
                    continue;
                }
                case 'table': {
                    // not impl yet
                ...
                case 'paragraph': {
                    ...
                    mdText += this.renderer.paragraph(this.parseInline(paragraphToken.tokens).mdText);
                    continue;
                }
                case 'katexblock': {
                    const katexToken = token;
    ...
    trimToken(mdText, displayText, markdownText, newDisplayText, isTrimed) {
        if (displayText.length - 1 >= 0
            && shouldAddSpaceBetweenTokens(displayText[displayText.length - 1], newDisplayText.trim()[0], isTrimed || newDisplayText !== newDisplayText.trimStart())) {
            mdText += ' ';
        }
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            switch (token.type) {
                case 'escape': {
                    ...
                    ({ mdText, displayText, isTrimed } = this.trimToken(mdText, displayText, renderedText, renderedText, isTrimed));
                    break;
                }
                case 'html': {
                ...
                case 'link': {
                    ...
                    const newMdText = renderer.link(linkToken.href, linkToken.title, linkChildText.mdText);
                    ({ mdText, displayText, isTrimed } = this.trimToken(mdText, displayText, newMdText, linkChildText.displayText, isTrimed || linkChildText.isTrimed));
                    break;
                }
                case 'image': {
                    const imageToken = token;
                    mdText += renderer.image(imageToken.href, imageToken.title, imageToken.text);
                    break;
                }
                case 'strong': {
                    ...
                    ({ mdText, displayText, isTrimed } = this.trimToken(mdText, displayText, newMdText, childText.displayText, isTrimed || childText.isTrimed));
                    break;
                }
                case 'em': {
                    const emToken = token;
                ...
                case 'codespan': {
                    ...
                    ({ mdText, displayText, isTrimed } = this.trimToken(mdText, displayText, newMdText, newMdText, isTrimed));
                    break;
                }
                case 'br': {
...
class SolutionFormatter {
    ...
    use(...args) {
        const extensions = this.defaults
        ...
        return (src, options) => {
            ...
            if (opt.async) {
                return Promise.resolve(opt.hooks ? opt.hooks.preprocess(src) : src)
                    ...
                    .then((tokens) => opt.walkTokens
                    ? Promise.all(this.walkTokens(tokens, opt.walkTokens)).then(() => tokens)
                    : tokens)
                    .then((tokens) => parser(tokens, opt))
                    .then((html) => opt.hooks ? opt.hooks.postprocess(html.mdText) : html.mdText)
                    .catch(throwError);
            }
            try {
                if (opt.hooks) {
...
export { _Hooks as Hooks, _Lexer as Lexer, _Parser as Parser, _Renderer as Renderer, SolutionFormatter, _TextRenderer as TextRenderer, _Tokenizer as Tokenizer, _defaults as defaults, _getDefaults as getDefaults, lexer, marked, options, parse, parseInline, parser, setOptions, use, walkTokens };
//# sourceMappingURL=lg-solution-formatter.esm.js.map
